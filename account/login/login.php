...
if ($stmt = $con->prepare("SELECT id, displayname, password, email, account_version, rounding, sorting, beta_tester, gradesystem FROM " . config_table_name_accounts . " WHERE username = ?")) {
    $stmt->bind_param('s', $username);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($id, $displayname, $password_hash, $email, $account_version, $setting_rounding, $setting_sorting, $beta_tester, $setting_system);
    $stmt->fetch();
